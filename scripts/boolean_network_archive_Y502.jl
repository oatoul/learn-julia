...
cfg = get_config("cfg/Y502.yaml")
mutate(ind::CGPInd) = goldman_mutate(cfg, ind)

print(cfg)

df_origin_bool = DataFrame(CSV.File("data/Y502_bool.tsv",drop=["Time"],type=Bool))
df_origin_int = DataFrame(CSV.File("data/Y502_bool.tsv",drop=["Time"],type=Int))
df_bool = copy(df_origin_bool)
insertcols!(df_bool, 1, :T0 => false, :T1 => true)

println(df_bool)

df_origin_float = DataFrame(CSV.File("data/Y502.tsv",drop=["Time"],type=Float32))
