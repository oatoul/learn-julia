...
function get_expect_Scerevisiae()
    res = Set()
    push!(res, "CDC28_CDC6")
    push!(res, "CDC28_SWI6")
...
cfg = get_config("cfg/Scerevisiae.yaml")
println(cfg)
mutate(ind::CGPInd) = goldman_mutate(cfg, ind)

df_origin_bool = DataFrame(CSV.File("data/Scerevisiae_bool.tsv",drop=["Time"],type=Bool))
df_bool = copy(df_origin_bool)
insertcols!(df_bool, 1, :T0 => false, :T1 => true)

println(df_bool)

df_origin_float = DataFrame(CSV.File("data/Scerevisiae.tsv",drop=["Time"],type=Float32))
