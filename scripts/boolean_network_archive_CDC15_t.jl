...
function evaluate_t!(ind::CGPInd, X::AbstractArray, Y::AbstractArray, tlag::Int)
    fit_max = evaluate!(ind, X, Y)
    tlag = tlag - 1
    x_ = copy(X)
    y_ = copy(Y)
    ...
    for i in 1:tlag
        pop!(x_)
        popfirst!(y_)
        fit_max = max(evaluate!(ind,x_,y_), fit_max)
    end
...
function getBN!(df::DataFrame, fitness::Float64, exptect_conn::Set, universe_conn::Set, tlag::Int)
    ...
    for i = l_idx:h_idx
        ...
        fit(ind::CGPInd) = evaluate_t!(ind, X, Y, tlag)
...
for i = 1:30
    ...
    BN_std, BN_fit, BN_mlp = getBN!(df_bool, 2.0, expect, universe, tlag)

    println("Structural acc for STD")
    evaluate_bn!(BN_std)
    # stru_acc = get_structural_accuracy(expect, BN_std.actual_conn, universe)
    dc1 = dynamic_consistency(df_origin_int, BN_std.actual_conn, tlag)
    ...
    dc2 = dynamic_consistency(df_origin_int, BN_fit.actual_conn, tlag)
    ...
    dc3 = dynamic_consistency(df_origin_int, BN_mlp.actual_conn, tlag)
